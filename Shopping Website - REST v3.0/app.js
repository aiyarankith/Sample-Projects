...
app.get('/cart.ejs', function(req, res){
	...
	for (var i=0; i<allcookies.length; i++){
		allcookies_arraylist[i] = {prod_name: allcookies[i].name, prod_id: allcookies[i].pr, prod_qty: allcookies[i].cc, prod_cost: allcookies[i].vv};
	}
	}
...
app.post('/add_to_cart', function(req, res){
	...
	mysql_cart.add_to_cart(function(err,results){
		if(err){
			...
			var values = {name: results[0].PRODUCT_NAME, pr: results[0].PRICE, cc: results[0].PRODUCT_ID, vv: QTY};
